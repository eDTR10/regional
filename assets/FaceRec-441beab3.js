import{o as Ue,a5 as oe,a6 as ie,a7 as Ke,a8 as We,a9 as Le,aa as Ve,ab as He,ac as Ge,ad as Je,ae as Ee,af as Xe,j as o,L as Ze,u as De,r,S as g,ag as Qe,z as W,ah as Pe,ai as Re,aj as et,ak as tt,a as ae}from"./index-8899cc8a.js";import{D as rt}from"./DashboardAnalogClock-315faec9.js";const st=Ue("RotateCcw",[["path",{d:"M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8",key:"1357e3"}],["path",{d:"M3 3v5h5",key:"1xhq8a"}]]);function nt(t,a){var c=Array.isArray(a)?a:[a];c.forEach(function(i){var l=i instanceof oe?i.score:ie(i)?i.detection.score:void 0,u=i instanceof oe?i.box:ie(i)?i.detection.box:new Ke(i),f=l?""+We(l):void 0;new Le(u,{label:f}).draw(t)})}function ot(t,a,c){c===void 0&&(c=!1);var i=c?Ve(a):a,l=i.width,u=i.height;return t.width=l,t.height=u,{width:l,height:u}}function Fe(t,a){var c=new He(a.width,a.height),i=c.width,l=c.height;if(i<=0||l<=0)throw new Error("resizeResults - invalid dimensions: "+JSON.stringify({width:i,height:l}));if(Array.isArray(t))return t.map(function(k){return Fe(k,{width:i,height:l})});if(Ge(t)){var u=t.detection.forSize(i,l),f=t.unshiftedLandmarks.forSize(u.box.width,u.box.height);return Je(Ee(t,u),f)}return ie(t)?Ee(t,t.detection.forSize(i,l)):t instanceof Xe||t instanceof oe?t.forSize(i,l):t}const it=()=>o.jsx("div",{className:"flex flex-col items-center justify-center h-screen w-full",children:o.jsxs("div",{className:"bg-card border border-border rounded-md p-8 shadow-md flex flex-col items-center",children:[o.jsx("p",{className:"text-2xl font-bold text-primary mb-2",children:"Face Registration Required"}),o.jsx("p",{className:"text-foreground text-center",children:"Please register your face from your profile to use this page."}),o.jsx(Ze,{to:"/regional/user/profile",className:" animate-bounce cursor-pointer text-primary text-xs",children:" View Profile →  Register your Face for Digital Biometric"}),o.jsx("p",{className:" text-xs text-yellow-500 mt-5",children:"*Note: Images won't be saved to the database — your privacy is safe 😊!"})]})}),V="location_permission_granted",H="camera_permission_granted",Ie="permissions_explained";function at(t,a,c,i){const u=(c-t)*Math.PI/180,f=(i-a)*Math.PI/180,k=Math.sin(u/2)*Math.sin(u/2)+Math.cos(t*Math.PI/180)*Math.cos(c*Math.PI/180)*Math.sin(f/2)*Math.sin(f/2);return 6371*(2*Math.atan2(Math.sqrt(k),Math.sqrt(1-k)))}function ct(t){return t.happy>.5}const ce=()=>typeof navigator<"u"&&/android|iphone|ipad|ipod|iemobile|mobile/i.test(navigator.userAgent),Me=()=>typeof navigator<"u"&&/chrome|crios/i.test(navigator.userAgent)&&!/edg|opr|brave|firefox/i.test(navigator.userAgent),lt=t=>t+(Me()?.01:0),Te=()=>{try{const t=navigator.deviceMemory??4,a=navigator.hardwareConcurrency??4;return ce()&&(t<=2||a<=4)||t<=2||a<=2?"low":t<=4||a<=4?"mid":"high"}catch{return"mid"}};function dt(t,a,c,i=.1){let l=1/0,u=!1,f="";const k=lt(i);return c.forEach(v=>{const[E,S]=v.split(",").map(x=>parseFloat(x.trim())),h=at(t,a,E,S);h<=k&&(u=!0),h<l&&(l=h,f=v)}),{isNearby:u,distance:l,nearestLocation:f}}function ut(){const t=new Date,a=t.getFullYear(),c=String(t.getMonth()+1).padStart(2,"0"),i=String(t.getDate()).padStart(2,"0"),l=String(t.getHours()).padStart(2,"0"),u=String(t.getMinutes()).padStart(2,"0"),f=String(t.getSeconds()).padStart(2,"0");return`${a}-${c}-${i}T${l}:${u}:${f}Z`}function mt(t,a){let c;return(...i)=>{clearTimeout(c),c=setTimeout(()=>t(...i),a)}}function ft(t,a){let c;return(...i)=>{c||(t(...i),c=!0,setTimeout(()=>c=!1,a))}}const b={hasStoredPermission:t=>localStorage.getItem(t)==="true",setPermissionStatus:(t,a)=>{localStorage.setItem(t,a.toString())},hasShownExplanation:()=>localStorage.getItem(Ie)==="true",setExplanationShown:()=>{localStorage.setItem(Ie,"true")},checkBrowserPermission:async t=>{try{return"permissions"in navigator?(await navigator.permissions.query({name:t})).state:null}catch(a){return console.warn(`Permission check failed for ${t}:`,a),null}}},G="/regional/models",Ne=t=>{const a=Te();let c={ideal:640,max:1280},i={ideal:480,max:720},l={ideal:15,max:30};return a==="low"?(c={ideal:480,max:640},i={ideal:360,max:480},l={ideal:12,max:24}):a==="mid"?(c={ideal:640,max:960},i={ideal:480,max:540},l={ideal:15,max:30}):(c={ideal:640,max:1280},i={ideal:480,max:720},l={ideal:24,max:30}),{video:{facingMode:t,width:c,height:i,frameRate:l,aspectRatio:1.333}}};function ht({userObject:t}){const a=De(),[c,i]=r.useState(!1),[l,u]=r.useState(!1),[f,k]=r.useState([]),[v,E]=r.useState([]),[S,h]=r.useState(null),[x,I]=r.useState(null),[z,N]=r.useState(null),[le,_]=r.useState("pending"),[Ae,F]=r.useState("Checking permissions..."),[de,L]=r.useState("Loading..."),[M,Be]=r.useState("user"),[ue,me]=r.useState([]),[fe,ze]=r.useState(!1),[O,J]=r.useState(!1),T=r.useRef(null),C=r.useRef(!1),$=r.useRef(null),X=r.useRef(!1),w=r.useRef(null),P=r.useRef(null),D=r.useRef(),q=r.useRef(null),A=r.useRef(),B=r.useRef(),he=r.useRef(),R=r.useRef(0),Z=le==="passed"&&C.current&&x==="ok"&&T.current===!0,ge=r.useCallback(()=>{if(!O&&!(Date.now()<R.current)){try{if(g.isVisible&&g.isVisible())return}catch{}J(!0),g.fire({title:"Biometric Actions",html:`
    ...
    `,showConfirmButton:!1,showCancelButton:!1,allowOutsideClick:!1,allowEscapeKey:!1,position:"center",toast:!1,timer:6e3,timerProgressBar:!0,width:"420px",background:"#fff",customClass:{popup:"floating-menu-popup",timerProgressBar:"floating-menu-timer"},didOpen:()=>{var e,n,s,d;(e=document.getElementById("time-in-btn"))==null||e.addEventListener("click",()=>{R.current=Date.now()+3e3,K("I"),g.close()}),(n=document.getElementById("break-in-btn"))==null||n.addEventListener("click",()=>{R.current=Date.now()+3e3,K("i"),g.close()}),(s=document.getElementById("break-out-btn"))==null||s.addEventListener("click",()=>{R.current=Date.now()+3e3,K("0"),g.close()}),(d=document.getElementById("time-out-btn"))==null||d.addEventListener("click",()=>{R.current=Date.now()+3e3,K("o"),g.close()})},willClose:()=>{R.current=Math.max(R.current,Date.now()+3e3),J(!1)}}),B.current=setTimeout(()=>{g.close(),J(!1)},4e3)}},[O]);r.useEffect(()=>(Z&&!O&&Date.now()>=R.current&&(!g.isVisible||!g.isVisible())&&ge(),()=>{B.current&&clearTimeout(B.current)}),[Z,O,ge]);const pe=r.useCallback(()=>{b.hasShownExplanation()?xe():g.fire({title:"Permission Setup Required",html:`
          ...
          `,icon:"info",confirmButtonText:"Grant Permissions",confirmButtonColor:"#3085d6",allowOutsideClick:!1,allowEscapeKey:!1}).then(()=>{b.setExplanationShown(),xe()})},[]),xe=r.useCallback(async()=>{L("Checking permissions...");const e=b.hasStoredPermission(H),n=b.hasStoredPermission(V),s=await b.checkBrowserPermission("camera"),d=await b.checkBrowserPermission("geolocation");e&&s==="granted"?I("ok"):s==="denied"?(I("permission_denied"),b.setPermissionStatus(H,!1)):await Q(),n&&d==="granted"?(h("ok"),C.current=!0):d==="denied"?(h("permission_denied"),C.current=!1,b.setPermissionStatus(V,!1)):await Y(),ze(!0)},[]),Q=r.useCallback(async()=>{I("checking");try{const e=Ne(M),n=await navigator.mediaDevices.getUserMedia(e);I("ok"),b.setPermissionStatus(H,!0),n.getTracks().forEach(s=>s.stop())}catch(e){e.name==="NotAllowedError"||e.name==="PermissionDeniedError"?(I("permission_denied"),b.setPermissionStatus(H,!1),_e()):(I("error"),console.error("Camera access error:",e))}},[M]),ee=r.useCallback(()=>{const e=window.location.origin,n=navigator.userAgent.toLowerCase(),s=n.includes("edg"),d=n.includes("firefox"),m=/android|iphone|ipad|ipod/i.test(n),y=(n.includes("chrome")||n.includes("crios"))&&!s&&!n.includes("opr");let p=null;if(!m&&(y||s)?p=`${s?"edge":"chrome"}://settings/content/siteDetails?site=${encodeURIComponent(e)}`:!m&&d&&(p="about:preferences#privacy"),p)try{window.open(p,"_blank")}catch{}const j=m?`<ol style="text-align:left; padding-left: 20px;">
            <li>Tap the lock icon in the address bar</li>
            <li>Permissions ➜ Location</li>
            <li>Choose "Allow while using"</li>
          </ol>`:`<p>In Site Settings, set Location to <strong>Allow</strong> for ${e}, then return here.</p>`;g.fire({icon:"info",title:"Enable location for this site",html:j,confirmButtonText:"Reload Page",allowOutsideClick:!1,allowEscapeKey:!1}).then(()=>window.location.reload())},[]),Y=r.useCallback(async()=>(h("checking"),navigator.geolocation?new Promise(e=>{navigator.geolocation.getCurrentPosition(n=>{h("ok"),C.current=!0,b.setPermissionStatus(V,!0),e("granted")},n=>{n.code===n.PERMISSION_DENIED?(h("permission_denied"),C.current=!1,b.setPermissionStatus(V,!1),te(),e("denied")):(h("error"),C.current=!1,console.error("Location access error:",n),e("error"))},{enableHighAccuracy:!1,timeout:1e4,maximumAge:0})}):(h("error"),N("❌ Geolocation not supported"),C.current=!1,"error")),[]),_e=r.useCallback(()=>{g.fire({title:"Camera Permission Required",html:`
          <div style="text-align: left; margin: 20px 0;">
        ...
        `,icon:"warning",confirmButtonText:"Grant Location Access",showCancelButton:!1,confirmButtonColor:"#3085d6"}).then(e=>{e.isConfirmed?b.checkBrowserPermission("geolocation").then(n=>{n==="denied"?ee():Y().finally(()=>window.location.reload())}).catch(()=>Y().finally(()=>window.location.reload())):e.isDenied&&ee()})},[Y,ee]),U=r.useCallback(ft(e=>{if($.current=e,!C.current){N("❌ Location permission required"),h("permission_denied");return}const n=()=>{if(!navigator.geolocation){N("❌ Geolocation not supported"),h("error");return}navigator.geolocation.getCurrentPosition(s=>{const{latitude:d,longitude:m}=s.coords,{isNearby:y,distance:p}=dt(d,m,e);T.current=y;const j=y?`✅ Within office range! (${p.toFixed(2)} km)`:`❌ Outside office range (${p.toFixed(2)} km from nearest office)`;N(j),h("ok")},s=>{if(console.error("Location error:",s),T.current!==null){const d=T.current?"✅ Within office range! (checking...)":"❌ Outside office range (checking...)";N(m=>!m||m.includes("Location permission required")||m.includes("Location access failed")?d:m)}else N("❌ Location access failed"),h("error")},{enableHighAccuracy:ce()||Me(),timeout:ce()?2e4:1e4,maximumAge:0})};A.current&&clearInterval(A.current),n(),A.current=setInterval(n,45e3)},15e3),[]),we=r.useCallback(async()=>{if(!l)try{L("Loading models...");try{const e=Qe;e!=null&&e.setBackend&&(await e.setBackend("webgl"),e!=null&&e.ready&&await e.ready())}catch{}await Promise.all([W.tinyFaceDetector.loadFromUri(G),W.faceLandmark68TinyNet.loadFromUri(G),W.faceRecognitionNet.loadFromUri(G),W.faceExpressionNet.loadFromUri(G)]);try{const e=document.createElement("canvas");e.width=128,e.height=128;const n=e.getContext("2d");n==null||n.fillRect(0,0,1,1),await Pe(e,new Re({inputSize:160,scoreThreshold:.9})).withFaceLandmarks(!0).withFaceDescriptors().withFaceExpressions()}catch{}u(!0)}catch{L("Error loading models")}},[l]),ye=r.useCallback(async()=>{try{const e=await Promise.all(f.map(n=>{const s=n.descriptors.map(d=>new Float32Array(d));return new et(n.label,s)}));q.current=new tt(e,.6)}catch(e){console.error("Face matcher initialization error:",e)}},[f]),be=r.useCallback(async()=>{var e;if(x!=="ok"){F("Camera permission required");return}try{(e=w.current)!=null&&e.srcObject&&(w.current.srcObject.getTracks().forEach(m=>m.stop()),w.current.srcObject=null);const n=Ne(M),s=await navigator.mediaDevices.getUserMedia(n);w.current&&(w.current.srcObject=s,w.current.onloadedmetadata=()=>{F("Please smile to verify liveliness")})}catch(n){console.error("Error starting video:",n),L("Error accessing camera"),I("error")}},[M,x]),ve=r.useCallback(()=>{if(!q.current||!w.current||!P.current||x!=="ok")return;D.current&&clearInterval(D.current);const e=Te(),n=e==="low"?224:e==="mid"?320:416,s=e==="low"?700:e==="mid"?500:350,d=async()=>{try{if(!w.current||!P.current||!q.current)return;const m=await Pe(w.current,new Re({inputSize:n,scoreThreshold:.6})).withFaceLandmarks(!0).withFaceDescriptors().withFaceExpressions(),y=P.current,p={width:500,height:600};ot(y,p);const j=Fe(m,p),re=y.getContext("2d",{willReadFrequently:!0});if(re&&re.clearRect(0,0,p.width,p.height),j.length>0){const se=j.map(ne=>q.current.findBestMatch(ne.descriptor));nt(y,j),se.forEach((ne,qe)=>{const Ye=j[qe].detection.box;new Le(Ye,{label:ne.toString()}).draw(y)});const $e=j[0].expressions,ke=se[0],Ce=ke&&ke.label!=="unknown",je=ct($e);Ce&&je?(_("passed"),F("Nice Smile!😉")):Ce&&!je?(_("pending"),F("Please smile.")):(_("pending"),F("Face not recognized.")),me(se),L("Running")}else _("pending"),F("No face detected"),me([])}catch(m){console.error("Face detection error:",m)}};D.current=setInterval(d,s)},[x]),Se=r.useCallback(async()=>{var e;try{const s=(await ae.get("users/userDetails",{headers:{Authorization:`Token ${localStorage.getItem("accessToken")}`}})).data;if(localStorage.setItem("user",JSON.stringify(s)),!(s!=null&&s.description))throw new Error("No face description data in API response");k([{label:s.full_name,descriptors:s.description}]),E([{id:s.full_name,name:s.full_name,position:s.job_title}]),i(!0),((e=s.location)==null?void 0:e.length)>0?($.current=s.location,U(s.location)):(N("❌ No office locations configured"),h("error"))}catch(n){L(n.message||"Error loading face data"),h("error")}},[U]),Oe={I:{message:"clocked in",emoji:"🌅"},i:{message:"started break",emoji:"☕"},0:{message:"ended break",emoji:"🍽️"},o:{message:"clocked out",emoji:"🌙"}},K=r.useCallback(mt(e=>{const{message:n,emoji:s}=Oe[e],d=ut();ae.post("checkinoutregion/create/",{CHECKTIME:d,CHECKTYPE:e,VERIFYCODE:t.deptid,SENSORID:t.deptid},{headers:{Authorization:`Token ${localStorage.getItem("accessToken")}`}}).then(()=>{const m={I:"Have a great day at work! 💼",i:"Enjoy your break! ☕",0:"Welcome back! 🔋",o:"See you tomorrow! 🌙"};g.fire({title:`${s} Success! ${s}`,html:`
              <div style="text-align: center; margin: 20px 0;">
                <p style="font-size: 18px; margin-bottom: 15px;">
                  You have successfully ${n}! 
                </p>
