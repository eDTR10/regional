...
function EditProfile({employee, getAllUsers, departments}:any) {
    ...
    // Only update face id
    function updateFaceId(faceId: any) {
        axios.put(`users/update/${employees.uid}/`, {
            ...
            Swal.fire({
                icon: "success",
                title: "Face ID Updated Successfully...",
            ...
            Swal.fire({
                icon: "error",
                title: "Oops...",
                text: error.response?.data?.non_field_errors?.[0] || "Failed to update face ID.",
                showConfirmButton: false,
            });
        });
    }
    ...
    function updateUser() {
        axios.put(`users/update/${employees.uid}/`,{
            ...
            Swal.fire({
                icon: "error",
                title: "Oops...",
                text: error.response?.data?.non_field_errors?.[0] || "Failed to update.",
                showConfirmButton: false,
            });
        })
    }
     ...
     function clearFaceId(e?: React.MouseEvent) {
        if (e) e.preventDefault(); // Prevent form submission or button default
        const confirmed = window.confirm("Are you sure? This will remove the employee's Face ID. This action cannot be undone.");
        if (confirmed) {
            axios.put(`users/update/${employees.uid}/`, {
                face_id: []
            }, {
                ...
                alert("Face ID Cleared!");
                getAllUsers();
            }).catch((error: any) => {
                alert(error.response?.data?.non_field_errors?.[0] || "Failed to clear face ID.");
            });
        }
    }
    return (
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            <DialogTrigger asChild>
                <PenIcon onClick={() => { setEmployees(employee) }} className="h-4 w-4 text-primary cursor-pointer"/>
            </DialogTrigger>
            <DialogContent className="sm:w-[90%] xxs:w-[90%] rounded-md ">
                ...
                {/* Add Face Registration Button */}
                <div className="mb-4  col-span-2">
                    <DrawerDemo {...employees} onFaceIdUpdate={updateFaceId} />
                    
                    <Button className=" mt-5"  variant="destructive" onClick={clearFaceId} >Remove Face ID</Button>
                </div>
                <form className={cn("grid items-start grid-cols-2 w-full p-2 flex-row gap-4 text-primary")} onSubmit={(e)=>{e.preventDefault(); updateUser();}}>
